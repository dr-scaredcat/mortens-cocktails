...
import { listSpiritTypes, type SpiritTypeRow } from "@/lib/spirits.functions";
import { upsertSpiritType, deleteSpiritType, reorderSpiritTypes } from "@/lib/admin.functions";
...
export function AdminSpiritTypes() {
  const qc = useQueryClient();
  const fetchList = useServerFn(listSpiritTypes);
  const upsert = useServerFn(upsertSpiritType);
  const del = useServerFn(deleteSpiritType);
  const reorder = useServerFn(reorderSpiritTypes);
  ...
  const [localTypes, setLocalTypes] = useState<SpiritTypeRow[] | null>(null);
  const types = localTypes ?? serverTypes ?? [];
  ...
  const invalidate = () => {
    setLocalTypes(null);
    qc.invalidateQueries({ queryKey: ["spirit-types"] });
  ...
  function sortAlpha() {
    const sorted = [...types].sort((a, b) => a.name.localeCompare(b.name, "da"));
    setLocalTypes(sorted);
  ...
  const addM = useMutation({
    ...
    onSuccess: () => {
      setName("");
      invalidate();
      toast.success("Spiritus type tilføjet");
    },
  ...
  return (
    <div className="space-y-4">
      <Card className="p-4">
        <h3 className="mb-3 font-medium">Tilføj spiritus type</h3>
          ...
          <ul className="divide-y divide-border rounded-lg border border-border bg-card">
            {types.length === 0 && (
              <li className="px-3 py-4 text-sm text-muted-foreground">
                Ingen spiritus typer endnu — tilføj en ovenfor.
              </li>
            )}
            {types.map((t) =>
              editingId === t.id ? (
                ...
                <SortableSpiritType
                  ...
                  onDelete={() => {
                    if (
                      confirm(
                        `Slet "${t.name}"? Spiritus med denne type mister deres type.`,
                      )
                    )
