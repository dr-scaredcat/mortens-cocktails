...
import { getMySpiritRatings, rateSpirit } from "@/lib/spirits.functions";
import { useRaterId } from "@/hooks/use-rater-id";
import { toast } from "sonner";

export function SpiritRatingStars({
  spiritId,
  avg,
  count,
}: {
  ...
  const fetchMine = useServerFn(getMySpiritRatings);
  const rate = useServerFn(rateSpirit);
  const [hover, setHover] = useState(0);

  const { data: mine } = useQuery({
    queryKey: ["my-spirit-ratings", raterId],
    queryFn: () => fetchMine({ data: { raterId: raterId! } }),
    enabled: !!raterId,
  });
  const myRating = mine?.find((r) => r.spirit_id === spiritId)?.rating ?? 0;

  const m = useMutation({
    mutationFn: (rating: number) => rate({ data: { spiritId, raterId: raterId!, rating } }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["spirits"] });
      qc.invalidateQueries({ queryKey: ["my-spirit-ratings", raterId] });
